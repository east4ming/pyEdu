...
> **目的**：便携式访问操作系统的特定功能。
> [原文链接](https://pymotw.com/3/os/index.html)
...
在fork之后，有两个进程运行相同的代码。对于一个程序来说明它是哪一个，它需要检查fork（）的返回值。如果值为0，则当前进程是子进程。如果不是0，程序在父进程中运行，返回值是子进程的进程号。

> 具体见`os_kill_example.py`

父进程可以使用kill（）和[信号signal模块](https://pymotw.com/3/signal/index.html#module-signal)向子进程发送信号。首先，定义一个信号处理程序在接收到信号时调用。然后fork（），在父节点暂停一小段时间，然后使用kill（）发送一个usr1信号。这个例子使用一个短暂的停顿来给子进程设置信号处理器的时间。一个真正的应用程序，不需要（或不想）调用sleep（）。在子进程身上，设置信号处理程序并休眠一段时间，给父进程时间发送信号。

在子进程中处理单独行为的简单方法是检查fork（）和分支的返回值。更复杂的行为可能需要比简单分支更多的代码分离。在其他情况下，可能存在需要包装的现有程序。对于这两种情况，可以使用exec *（）系列函数来运行另一个程序。

> 见`os_exec_example.py`

exec（）的种类很多，具体取决于参数可用的形式，是否将父进程的路径和环境复制到子进程等等，对于所有变体，第一个参数是路径或文件名其余的参数控制着程序的运行方式。它们要么作为命令行参数传递，要么覆盖进程“环境”（参见os.environ和os.getenv）。有关完整的详细信息，请参阅库文档。

## 等待子进程

许多计算密集型程序使用多个进程来解决python和全局解释器锁的线程限制。当启动多个进程来运行单独的任务时，在启动新的进程之前，主服务器需要等待一个或多个进程完成，以避免服务器超载。有几种不同的方式来使用wait（）和相关函数。
当哪个子进程可能首先退出无关紧要时，使用wait（）。它会在任何子进程退出时立即返回。

> 具体见`os_wait_example.py`

wait（）的返回值是一个包含进程ID和退出状态的元组，组合成一个16位的值。低字节是杀死进程的信号的编号，高字节是进程退出时返回的状态码。

等待一个特定的进程，使用waitpid（）。

> 具体见`os_waitpid_example.py`

传递目标进程的进程ID，并且waitpid（）阻塞直到该进程退出。
wait3（）和wait4（）以类似的方式工作，但返回有关子进程的更多详细信息，包括pid，退出状态和资源使用情况。

## 产生新的进程

作为一种方便，spawn（）系列函数在一个语句中处理fork（）和exec（）：

> 具体见`os_spawn_example.py`

第一个参数是一个模式，指示是否在返回之前等待进程完成。这个例子等待。使用p_nowait让其他进程启动，但在当前进程中恢复。

## 操作系统错误代码

由操作系统定义并在errno模块中管理的错误代码可以使用strerror（）转换为消息字符串。
